...
const TvDetailComponent = () => {
    ...
    const matchingWishes = wishList.filter((item: any) => item.content_id === tvDetail.id);
    const singleWish = matchingWishes.length > 0 ? matchingWishes[0] : null;

    const matchingWished = wishedList.filter((item: any) => item.content_id === tvDetail.id);
    ...
    return (
        <div className="detail_component">
            ...
            <div className="content_wrap1"
                 style={{backgroundImage: `url(${IMAGE_BASE_URL + tvDetail?.backdrop_path})`}}>
